/******************************************************************************
   EXAMPLE: adult_mitigate_bias.sas
   ...
   PURPOSE: In this example, we build a Random Forest classification model
            ...
            score the test data using the saved ASTORE and assess the model
            for bias across the sensitive variable SEX using PROC ASSESSBIAS.
            Finally, we retrain the Random Forest model with the MITIGATEBIAS
            statement to produce a fairer model that reduces disparate impact
            across SEX while maintaining predictive performance.
******************************************************************************/
...
/******************************************************************************
   Retrain the Random Forest model with bias mitigation applied.

   Having identified disparate impact across SEX in the baseline model, we
   retrain PROC FOREST with the MITIGATEBIAS statement. This instructs the
   procedure to adjust the training process so that predicted probabilities
   are more equitable across the two SEX groups, while still optimizing for
   overall predictive accuracy. The seed ensures reproducibility of the
   mitigation adjustments.

   - PEVENTS identifies the predicted probability columns for each class.
   - PVARS maps those probability columns to the corresponding class labels.
   - SENSITIVEVAR designates SEX as the protected demographic attribute.
   - TARGETEVENT specifies which outcome class is the focus of fairness review.
******************************************************************************/

title2 'FOREST with bias mitigation on adult_train data';
...
proc forest data=adult_train seed=12345;
    ...
    input workclass education marital_status occupation
          relationship race sex native_country / level=nominal;
    target target / level=nominal;            /* Binary income target variable  */
    savestate rstore=dtstore;                 /* Overwrite ASTORE with fair model */

    /* Apply in-processing bias mitigation during model training             */
    mitigateBias pevents=">50K <=50K"         /* Predicted probability columns   */
                 pvars=(P_target_50K P_target__50K)  /* Class probability vars   */
                 seed=12345                   /* Seed for reproducible mitigation */
                 sensitivevar=sex             /* Protected demographic attribute  */
                 targetEvent=">50K";          /* Positive event for fairness focus */
run;